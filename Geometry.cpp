...
public:
    ...
    void setValue(const Real& newValue) {
        value = newValue;
    }
...
public:
    ...
    EpsReal length() const {
        return EpsReal(sqrt((Real)(x * x + y * y)));
    }
    ...
    Vector2<T> leftPerpendicular() const {
        return {-y, x};
    }
    Vector2<T> rightPerpendicular() const {
        return {y, -x};
    }
};
...
template<typename T> EpsReal distance(const Point2<T>& A, const Point2<T>& B) {
    return (B - A).length();
}
 ...
 */
template<typename T> EpsReal polar(const Vector2<T>& u, const Vector2<T>& v) {
    return atan2((Real)(u % v), (Real)(u * v));
}
/**
 * Returns polar angle in [0; 2*PI].
 */
template<typename T> EpsReal polarPositive(const Vector2<T>& u, const Vector2<T>& v) {
...
public:
    ...
    Vector2<T> operator*(const Vector2<T>& v) const {
        return Vector2<T>(value[0][0] * v[0] + value[0][1] * v[1], value[1][0] * v[0] + value[1][1] * v[1]);
    }
    Matrix2<T> operator*(const Matrix2<T>& other) const {
        Matrix2<T> res; // null
        for (int i = 0; i < 2; ++i)
...
template<typename T> Matrix2<T> operator*(const T& k, const Matrix2<T>& M) {
...
public:
    ...
    Line(const Vector2<T>& _normal, const T& _value): normal(_normal), value(_value) {}
    Line(const T& a, const T& b, const T& c): normal(Vector2<T>(a, b)), value(c) {}
    template<typename U> Line(const U& a, const U& b, const U& c): normal(Vector2<T>((U)a, (U)b)), value(c) {}
    Line(const Line<T>& other): normal(other.normal), value(other.value) {}
    template<typename U> Line(const Line<U>& other): normal(Vector2<T>(other.normal)), value((T)other.value) {}
    ~Line() {}
...
template<typename T>
LineIntersectionResult lineIntersection(const Line<T>& lhs, const Line<T>& rhs) {
    EpsReal D  = Matrix2(lhs.normal.x, lhs.normal.y,
                            rhs.normal.x, rhs.normal.y).det();
    EpsReal Dx = Matrix2(lhs.value, lhs.normal.y,
                            rhs.value, rhs.normal.y).det();
    EpsReal Dy = Matrix2(lhs.normal.x, lhs.value,
                            rhs.normal.x, rhs.value).det();
    if (D == 0) {
        if (Dx == 0)
            return {LineIntersectionSize::INF, RealPoint()};
...
public:
    ...
    template<typename U> Segment(const Segment<U>& other): A((Point2<T>)other.A), B((Point2<T>)other.B) {}
    ~Segment() {}
...
template<typename T> ostream& operator<<(ostream& stream, const Segment<T>& segment) {
    return stream << segment.A << ' ' << segment.B;
}
template<typename T> istream& operator>>(istream& stream, Segment<T>& segment) {
    return stream >> segment.A >> segment.B;
}
